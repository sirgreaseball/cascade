...
const ARCHITECTURE = [
  {
    title: 'DUAL SOLVER CORE',
    description: 'A shallow-water finite-volume solver and an SPH particle solver run side by side in Web Workers, with no server or dedicated hardware.',
    icon: '01',
    status: 'ACTIVE',
    popupText: 'SYSTEM LOG: HLL flux solver and SPH particles initialised from the same breach hydrograph. Mass balance holds to machine precision.',
  },
  {
    title: 'SATELLITE VALIDATION',
    description: 'Sentinel-1 SAR change detection on Google Earth Engine maps the real flood extent and scores the model against it.',
    icon: '02',
    status: 'READY',
    popupText: 'SYSTEM LOG: Earth Engine script generated for the study area. Before/after SAR composites ready for near-real-time flood mapping.',
  },
  {
    title: 'EXPOSURE & LOSS',
    description: 'OpenStreetMap roads, bridges and facilities checked against depth and velocity as the flood advances, with hazard grades and rupee loss estimates.',
    icon: '03',
    status: 'ONLINE',
    popupText: 'SYSTEM LOG: Exposure layer loaded from OpenStreetMap. Assets graded H1–H6, losses from JRC depth-damage curves.',
  }
];
        ...
        <div className="mb-24 text-center md:text-left flex flex-col md:flex-row justify-between items-end border-b border-white/10 pb-8">
          <div>
            ...
            <h2 className="font-serif italic text-4xl md:text-6xl tracking-tight max-w-3xl text-white leading-none">
              Research-grade hydraulics,<br />running in your browser.
            </h2>
          </div>
          <div className="hidden md:block text-right font-mono text-[10px] text-dim tracking-widest uppercase">
            <p>COMPUTE: WEB WORKERS, ON DEVICE</p>
            <p>DATA: SRTM · OPENSTREETMAP · SENTINEL-1</p>
          </div>
        </div>
